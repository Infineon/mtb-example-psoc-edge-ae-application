...
3. Ensure to set the following in common.mk (either ARM or LLVM_ARM for TOOLCHAIN)
...
In common.mk, set CONFIG_VOICE_CORE_MODE=FULL if the unlimited licensed version of audio-voice-core library is obtained, else do not change it.
If the licensed version is got, then place COMPONENT_AVC_FULL folder inside `proj_cm55\source\modules\audio_voice_core_lib` folder. 

<br>
The code example supports two modes "FUNCTIONAL" and "TUNING".

<br>
 The default mode is "FUNCTIONAL" which is set in common.mk. set CONFIG_AE_MODE=FUNCTIONAL 
 In this mode, the kit will enumerate as a single channel USB mic.
<br>
...
4. In "TUNING" mode (CONFIG_AE_MODE=TUNING), the kit will enumerate as quad channel USB mic. <br>

5. Build the code. <br>

6. You can build the application using the following methods:
